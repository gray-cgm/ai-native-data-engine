...
function resolveErrorResponse(error: unknown) {
  if (isValidationError(error)) {
    ...
    return {
      status,
      code:
        status >= 500 ? errorCodes.router.responseFieldIncorrect : errorCodes.router.requestParamIncorrect,
...
function isValidationError(error: unknown): error is { name?: string; status?: number; details?: Array<{ message?: string }> } {
  return Boolean(error && typeof error === 'object' && (error as { name?: string }).name === 'ValidationError')
}

function getValidationErrorMessage(error: { details?: Array<{ message?: string }> }) {
