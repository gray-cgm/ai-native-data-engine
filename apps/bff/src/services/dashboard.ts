...
export async function buildDashboardPayload(): Promise<DashboardPayload> {
  const [distributionRes, datasetsRes, tasksRes, workspacesRes, exportsRes, searchRes, runsRes, streamingRes] = await Promise.all([
    platformFetch('/samples/distribution').catch((err) => { logger.error('dashboard fetch failed', { endpoint: '/samples/distribution', error: String(err) }); return { distribution: [], scenario: null } }),
    platformFetch('/datasets').catch((err) => { logger.error('dashboard fetch failed', { endpoint: '/datasets', error: String(err) }); return { items: [] } }),
    platformFetch('/tasks').catch((err) => { logger.error('dashboard fetch failed', { endpoint: '/tasks', error: String(err) }); return { items: [] } }),
    platformFetch('/workspaces').catch((err) => { logger.error('dashboard fetch failed', { endpoint: '/workspaces', error: String(err) }); return { items: [] } }),
    platformFetch('/exports').catch((err) => { logger.error('dashboard fetch failed', { endpoint: '/exports', error: String(err) }); return { items: [] } }),
    platformFetch('/samples/search-preview').catch((err) => { logger.error('dashboard fetch failed', { endpoint: '/samples/search-preview', error: String(err) }); return { rows: [] } }),
    platformFetch('/runs').catch((err) => { logger.error('dashboard fetch failed', { endpoint: '/runs', error: String(err) }); return { items: [] } }),
    platformFetch('/streaming/summary').catch((err) => { logger.error('dashboard fetch failed', { endpoint: '/streaming/summary', error: String(err) }); return { summary: null } }),
  ])
